...
function linearRegression(dataSet, iterations, learnRate, initialTheta0 = Math.random(), initialTheta1 = Math.random()){
	...
	console.log("trainingData length: " + batchSize)
	...
	*/
	...
	// Main loop to run through each iteration
	for(let i = 0; i < iterations; i++){
		//let trainingData = linearRegressionGenerateBatch(batchSize, standardizedData);// For using data standartization (not implemented yet)
		let trainingData = linearRegressionGenerateBatch(batchSize, dataSet);// Generate random training batch
		//trainingData.sort((a, b)=>{return Math.random() - 0.5;});// Shuffle array (not needed as randomization implemented in batch data generation function
		previousCost = cost;
		cost = 0;
		let deltaTheta0 = 0;
		let deltaTheta1 = 0;
		...
		theta0 = theta0 - ((learnRate * deltaTheta0) / (trainingData.length));// If data values are big (not normalised/standardised), this converges very slowly and learnRate should be changed to 1. This needs looking more into.
...
// random integer generator up to, but not including the supplied parameter value
function linearRegressionRandomInteger(maxSize){
	return Math.floor( Math.random() * maxSize );
};
// Take a small batch from data set to use as training or validation
function linearRegressionGenerateBatch(trainingBatchSize, trainingData){
	let miniBatch = [];
	for(let i = 0; i < trainingBatchSize; i++){
		miniBatch.push(trainingData[linearRegressionRandomInteger(trainingData.length)])// Picks random elements from data set
	};
	return miniBatch;
};
// Data standardization (not tested yet)
